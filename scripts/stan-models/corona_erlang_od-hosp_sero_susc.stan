 ...
 * The infectious period is Erlang-distributed.
 */
...
functions {
  ...
  vector corona_model(real t, vector y, real epsilon, real alpha, real gamma, 
                      ...
                      int[] susc_classes, int[] hosp_classes, int A, int J) {
    vector[(2+J)*A] dydt;
    real q = inv_logit(k * (t-x0));
    matrix[A,A] C = (1-q) * Cunp + q * zeta * Cdis;
    vector[A] incidence;
    vector[A] Itot = y[2*A+1:3*A];
    for ( j in 2:J ) { // add other I-stages to Itot if J > 1
      Itot += y[(j+1)*A+1:(j+2)*A];
    }
    incidence = (C * Itot * epsilon) .* beta_short[susc_classes] .* y[1:A];
    dydt[1:A] = -incidence;                                  // Susceptible        
    dydt[A+1:2*A] = incidence - alpha * y[A+1:2*A];          // Exposed
    // first infectious stage
    dydt[2*A+1:3*A] = alpha * y[A+1:2*A] - (gamma*J + nu_short[hosp_classes]) .* y[2*A+1:3*A];
    // other infectous stages (if J > 1)
    for ( j in 2:J ) {
      dydt[(1+j)*A+1:(2+j)*A] = gamma*J * y[j*A+1:(j+1)*A] 
        - (gamma*J + nu_short[hosp_classes]) .* y[(1+j)*A+1:(2+j)*A]; // Infectious
    }
    return dydt;
  }
}
...
parameters {
  ...
  real<lower=1e-7, upper=5e-4> inoculum;                           // approx 0.1-10k initial infections
...
transformed parameters {
  ...
  for (a in 1:A) { // TODO: extend/include long run-up period to approach stable class distribution
    ...
    y0[A+a] = 0.5 * inoculum;                        // Exposed
    for ( j in 1:J ) {
      y0[(1+j)*A+a] = 0.5/J * inoculum;              // Infected
    }
  }
  ...
  y_hat = ode_adams_tol(corona_model, y0, t0, ts_all[sort_indices], 
      ...
      susc_classes, hosp_classes, A, J);
  
  /* extract trajectories */
  for ( i in 1:numdays_all ) {
      Susceptible[i,:] = y_hat[i][1:A]';         
      Exposed[i,:] = y_hat[i][A+1:2*A]';
      for ( j in 1:J ) {
        Infected[i,:] += y_hat[i][(1+j)*A+1:(2+j)*A]';
      }
  }
...
model {
  ...
  nu_short ~ normal(0, 5); // vague prior. NB nu is small
